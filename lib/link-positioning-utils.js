module.exports = (saveFilepath) => {
    ...
    const LinkPositioningUtils = {
        getDegreesFromSouth: () => {
            const degrees = float28.decodeFloat(offsetChecker(0x0006ea68, saveFilepath));
            return degrees;
        },
        getPosition: () => {
            const x = float28.decodeFloat(offsetChecker(0x000c3bf0, saveFilepath));
            const y = float28.decodeFloat(offsetChecker(0x000c3c00, saveFilepath));
            const z = float28.decodeFloat(offsetChecker(0x000c3bf8, saveFilepath));
            return { x, y, z };
        },
        getRadiansFromSouth: () => {
            const degrees = float28.decodeFloat(offsetChecker(0x0006ea68, saveFilepath));
