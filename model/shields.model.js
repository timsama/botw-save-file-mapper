        ...
        write: (modelJson, saveFile, startingSlot, options, effectMapPath) => {
            ...
            const quickTipsPromise = (() => {
                if ((!options || !options.skipSoftDependencies) && modelJson.slots.length > 1) {
                    ...
                    return writeChanges(['quicktips.switchshields.viewed'], options);
                } else {
                    return Promise.resolve();
                }
            })();

            return quickTipsPromise.then(() => writeItemSlots(saveFile, modelJson.slots, startingSlot, 'shields', options, (item, slot, slotInCategory) => {
                const equippedOffset = Offsets.getEquippedSlotOffset(slot);
                const durabilityOffset = Offsets.getQuantitiesOffset(slot);
                const typeOffset = Offsets.getBonusTypeOffset(slotInCategory, 'shields');
                const amountOffset = Offsets.getBonusAmountOffset(slotInCategory, 'shields');
